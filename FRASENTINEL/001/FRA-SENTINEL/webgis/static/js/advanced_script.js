...
class FRAAtlas {
    ...
    displayRecommendations(data) {
        ...
        data.recommendations.forEach((rec, index) => {
            const priority = rec.dajgua_priority || rec.priority || 'Medium';
            ...
            html += `
                <div class="recommendation-item enhanced-rec" style="animation-delay: ${index * 0.1}s;">
                    ...
                    <div class="rec-content">
                        <p class="scheme-benefit">💰 ${rec.benefit_amount || 'N/A'}</p>
                        <p class="scheme-ministry">🏛️ ${rec.ministry || 'N/A'}</p>
                        <div class="scheme-reasons">
                            ${(rec.reason || '').split(', ').map(reason => `<span class="reason-tag">${reason}</span>`).join('')}
                        </div>
    ...
    async loadProgressData(scope = 'state', id = '') {
        try {
            const response = await fetch(`/api/progress?scope=${scope}&id=${id}`);
            const data = await response.json();
    ...
    updateProgressStats(data) {
        ...
        document.getElementById('ifrStats').innerHTML = `
            ${aggregates.ifr_granted}/${aggregates.ifr_total} (${rates.ifr_rate}%)
        `;
        
        document.getElementById('crStats').innerHTML = `
            ${aggregates.cr_granted}/${aggregates.cr_total} (${rates.cr_rate}%)
        `;
        
        document.getElementById('cfrStats').innerHTML = `
            ${aggregates.cfr_granted}/${aggregates.cfr_total} (${rates.cfr_rate}%)
        `;
    }
    ...
    updateProgressChart(data) {
        ...
        // IFR Bar
        const ifrTotal = aggregates.ifr_total;
        const ifrGranted = aggregates.ifr_granted;
        const ifrHeight = (ifrTotal / Math.max(ifrTotal, aggregates.cr_total, aggregates.cfr_total)) * maxHeight;
        const ifrGrantedHeight = (ifrGranted / ifrTotal) * ifrHeight;
        
        // Draw IFR total (light green)
        ctx.fillStyle = '#e8f5e8';
        ctx.fillRect(startX, startY - ifrHeight, barWidth, ifrHeight);
        
        // Draw IFR granted (dark green)
        ctx.fillStyle = '#4caf50';
        ctx.fillRect(startX, startY - ifrGrantedHeight, barWidth, ifrGrantedHeight);
        
        // CR Bar
        const crTotal = aggregates.cr_total;
        const crGranted = aggregates.cr_granted;
        const crHeight = (crTotal / Math.max(ifrTotal, crTotal, aggregates.cfr_total)) * maxHeight;
        const crGrantedHeight = (crGranted / crTotal) * crHeight;
        
        ctx.fillStyle = '#e8f5e8';
        ctx.fillRect(startX + barWidth + barSpacing, startY - crHeight, barWidth, crHeight);
        
        ctx.fillStyle = '#2e7d32';
        ctx.fillRect(startX + barWidth + barSpacing, startY - crGrantedHeight, barWidth, crGrantedHeight);
        
        // CFR Bar
        const cfrTotal = aggregates.cfr_total;
        const cfrGranted = aggregates.cfr_granted;
        const cfrHeight = (cfrTotal / Math.max(ifrTotal, crTotal, cfrTotal)) * maxHeight;
        const cfrGrantedHeight = (cfrGranted / cfrTotal) * cfrHeight;
        
        ctx.fillStyle = '#e8f5e8';
        ctx.fillRect(startX + 2 * (barWidth + barSpacing), startY - cfrHeight, barWidth, cfrHeight);
        
        ctx.fillStyle = '#0d4f3c';
        ctx.fillRect(startX + 2 * (barWidth + barSpacing), startY - cfrGrantedHeight, barWidth, cfrGrantedHeight);
        ...
        ctx.fillText('IFR', startX + barWidth/2, startY + 20);
        ctx.fillText('CR', startX + barWidth + barSpacing + barWidth/2, startY + 20);
        ctx.fillText('CFR', startX + 2 * (barWidth + barSpacing) + barWidth/2, startY + 20);
    }

    updateTopPending(data) {
        const pendingList = document.getElementById('pendingList');
        const topPending = data.top_pending_villages;
        
        if (topPending.length === 0) {
            pendingList.innerHTML = '<div class="pending-item">No pending villages found</div>';
            return;
        }
        
        let html = '';
        topPending.forEach(village => {
            html += `
                <div class="pending-item" onclick="fraAtlas.navigateToVillage('${village.village}')">
                    <div class="pending-village">
                        ${village.village}<br>
                        <small>${village.district}, ${village.state}</small>
                    </div>
                    <div class="pending-count">${village.total_pending}</div>
                </div>
            `;
        });
    ...
    initBreadcrumbNavigation() {
        ...
        breadcrumbNav.addEventListener('click', (e) => {
            if (e.target.classList.contains('breadcrumb-item')) {
                ...
                this.loadProgressData(scope, id);
                this.updateBreadcrumb(scope, id);
            }
        });
    }

    updateBreadcrumb(scope, id) {
        const breadcrumbNav = document.getElementById('breadcrumbNav');
        
        // Update breadcrumb based on scope
        let html = '';
        if (scope === 'state' && id) {
            html = `
                <span class="breadcrumb-item" data-scope="state" data-id="">All States</span>
                <span class="breadcrumb-item active" data-scope="state" data-id="${id}">${id}</span>
            `;
        } else if (scope === 'district' && id) {
            // This would need to be enhanced with actual district data
            html = `
                <span class="breadcrumb-item" data-scope="state" data-id="">All States</span>
                <span class="breadcrumb-item" data-scope="state" data-id="Madhya Pradesh">Madhya Pradesh</span>
                <span class="breadcrumb-item active" data-scope="district" data-id="${id}">${id}</span>
            `;
        } else {
            html = '<span class="breadcrumb-item active" data-scope="state" data-id="">All States</span>';
        }
        
        breadcrumbNav.innerHTML = html;
    }

    navigateToVillage(villageName) {
        // Zoom to village on map
        if (this.map) {
            // Find village coordinates and zoom
            const village = TEST_VILLAGES.features.find(f => f.properties.village === villageName);
            if (village) {
                this.map.setView([village.properties.latitude, village.properties.longitude], 15);
            }
        }
    }
}

// Global functions for resources drawer
function toggleResourcesDrawer() {
...
function downloadGuidelines(type) {
    if (type === 'fra') {
        alert('FRA Guidelines PDF would be downloaded here. This is a demo implementation.');
    } else if (type === 'dajgua') {
        alert('DAJGUA Guidelines PDF would be downloaded here. This is a demo implementation.');
    }
}
...
document.addEventListener('click', function(event) {
    ...
    if (drawer && drawer.classList.contains('open')) {
        if (!drawer.contains(event.target) && !resourcesBtn.contains(event.target)) {
            drawer.classList.remove('open');
        }
    }
});
